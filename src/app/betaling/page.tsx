...
import { createMembership, registerCard, setMembershipFrequency } from "@/server/memberships/actions";
import { getActivePricingRule } from "@/server/pricing";
import { formatMinor } from "@/server/pricing/algorithm";

export const metadata: Metadata = { title: "Betaling — Advisory Board Unlimited" };
...
export default async function PaymentPage() {
  ...
  return (
    <main className="container stack">
      <p className="eyebrow">
        <Link href="/dashboard">Dashboard</Link> · Betaling
      </p>
      <h1 className="heading-2 heading--on-light">Betaling</h1>
      <p className="body">
        Dit kort registreres ved booking, og betalingen trækkes først når et møde er afholdt —
        med varierende betalingsfrekvenser (hver 4., 8. eller 12. uge). Ændrer du board eller
        frekvens, gælder den nye pris fra næste afholdte møde.
      </p>
      ...
      {!board ? (
        ...
        <>
          <section className="stack measure">
            ...
            {rule ? (
              <PriceBreakdown rule={rule} partnerCount={partnerCount} frequencyWeeks={frequency} />
            ) : (
          ...
          <section className="stack measure">
            ...
            {!membership ? (
              <AuthForm action={createMembership} submitLabel="Opret medlemskab">
                ...
                <Select name="frequency_weeks" label="Mødefrekvens" defaultValue={String(frequency)}>
                  ...
                  {charges.map((charge) => (
                    <tr key={charge.id} className="table__row">
                      <td className="table__cell">
                        {new Date(charge.createdAt).toLocaleDateString("da-DK", { dateStyle: "long" })}
                      </td>
                      <td className="table__cell">{formatMinor(charge.amountMinor, charge.currency)}</td>
                      <td className="table__cell">
