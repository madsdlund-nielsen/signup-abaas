...
> **Præmis-rettelse (2026-09-15, ADR 0046).** Formålet stod oprindeligt som "via managed
...
| Plan/pris-binding | ✅ **lukket** | **Teams, $12/bruger/md** (Mads, 2026-09-15; ADR 0046). Platform er lukket for nye kunder. Sæder = rådgivere + admin | ejer/Mads beslutter |
...
(ADR 0046). **Dataresidens-delen består** — EU-residens på Teams er uverificeret (L-7) og er
...
|---|---|---|---|
| L-1 | **Multi-host:** 2-3 partnere som værter + ejer som attendee på ét event, mod boardets **collective** team event type (ADR 0046) | Kræver konto | `createMultiHostMeeting` (`src/lib/booking/calcom.ts`) |
| ~~L-2~~ | ✅ **LUKKET 2026-09-15 uden konto** (ADR 0046). Svaret er "ingen af delene": `POST /bookings` har **intet `hosts`-felt** — værter hører til event typen — og `attendee` er **ét objekt** med `{name, email, timeZone}`, ikke et bruger-id. Gættet var forkert, og ingen plan kan købe os ud af det | Besvaret af leverandørens API-reference; krævede ingen konto | `createMultiHostMeeting`, `readBoardBookingInfo` → rework sporet som `docs/backlog.md` **B-23** |
...
| L-7 | **EU-residens** på **Teams** — og om `apiUrl` skal være `cal.eu` eller self-host | Plan-valget (ADR 0046) svarer **ikke** på dataresidens; den del af STOP-gaten består | `CALCOM_API_URL`, `docs/gdpr/leverandoer-register.md` |
...
**Når checklisten køres:** ADR 0046 dækker plan-valget og booking-modellen. Skriv en
