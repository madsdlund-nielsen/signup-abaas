...
> **Status (2026-07-22, ADR 0019):** leveres i 2 PR'er. **PR 1:** migration 0009 (`partner_profile`
> afkoblet fra auth + `partner_profile_competence_tag` M2M + admin-only RLS) + `src/server/partners`
> (reads + admin-CRUD + `setPartnerTags`). **PR 2:** admin-UI `/admin/partners` (liste/opret/redigér +
> checkbox-tag-picker). **Scope:** kun det admin-forfattede katalog (det 1.5 matcher imod). Kun sikre
