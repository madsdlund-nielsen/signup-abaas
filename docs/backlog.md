...
| **B-27** | **Hver sidevisning koster en serverfunktion — intet caches.** Målt på `sua-abaas.netlify.app` 2026-09-21: kold TTFB på `/` **2,60 s**, varm **0,31-0,66 s**. Svarene bærer `no-store` og rammer hverken edge- eller durable-cache. ~~**(1)** duplikerede brugeropslag pr. request~~ og ~~webhooks bag adgangsporten~~ → **lukket af ADR 0049** (memoisering pr. request: 2 opslag → 1 på `/dashboard` og `/moeder`, målt; `api` ude af matcheren). **Tilbage står to ting:** **(a)** `SiteHeader` ligger i **rodlayoutet** og venter på `getCurrentUser()`, hvilket gør **hver** rute dynamisk — også forsiden, `/login` og `/styleguide`, der ellers prærenderes statisk. Det er grunden til at intet kan caches, og det er den største resterende gevinst for en førstegangsbesøgende. **(b)** Funktionerne kører i **US East (Ohio)** fordi funktions-regionen ikke er sat, mens Supabase ligger i **eu-north-1 (Stockholm)**. | (a) `src/components/SiteHeader.tsx`, `src/app/layout.tsx` — rører site-kromet (ADR 0039); (b) `netlify.toml`, Netlify-plan | **Ejerne dømmer platformen på om den føles high-end (CLAUDE.md § Design) — det gør en 2,6 s kold forside ikke.** (a) er vores kode og kan rettes uden at købe noget, men er en kromændring der fortjener sin egen skive — ikke en sidegevinst. (b) kræver Netlify Pro og skal alligevel træffes for EU-residens-kravet; den er **ikke** Claude Codes valg. ⚠ Målingerne er taget gennem en agent-proxy, ikke fra en dansk forbindelse; de relative forhold holder, de absolutte tal er formentlig bedre end det Andreas ser. Mål igen fra DK før der konkluderes på tal. | Mads (a) + ejer/Mads (b: plan) |
