...
| **B-27** | **Intet caches — hver sidevisning koster en serverfunktion.** Målt på `sua-abaas.netlify.app` 2026-09-21: kold TTFB på `/` **2,60 s**, varm **0,31-0,66 s**, alt med `no-store`. ~~Duplikerede brugeropslag~~ → lukket af **ADR 0049** (2 opslag → 1, målt). ~~Hele siden ventede på sessionen før første byte~~ → lukket af **ADR 0050**: `SiteHeader` er auth-fri, nav'en streames — TTFB på `/` **162 ms → 15 ms** med en simuleret rundtur på 150 ms. **Tilbage står:** **(a)** ruterne er stadig **dynamiske**, så der caches intet på CDN'en. Årsagen er nu isoleret, og vejen videre er Next 16's **Cache Components** (`cacheComponents: true` → PPR: statisk skal + streamede huller). **Afprøvet:** buildet fejler på **19** filer med `Route segment config "dynamic" is not compatible with nextConfig.cacheComponents` — de bevidste `force-dynamic`-markeringer på de session-afhængige ruter. Migrationen vender cache-modellen om og kræver en Suspense-gennemgang af hver authed side. **(b)** Funktionerne kører i **US East (Ohio)**, mens Supabase ligger i **eu-north-1 (Stockholm)**; funktions-regionen kræver Netlify Pro. | (a) `next.config.ts` + de 19 `export const dynamic`-linjer; (b) `netlify.toml`, Netlify-plan | **(a) er ikke Claude Codes valg alene** — det er hele rammeværkets cache-semantik og har egen risiko på Netlify. Beslutningsgrundlaget er nu målt frem for gættet (se ADR 0050). **(b)** kræver Pro og skal alligevel træffes for EU-residens-kravet. ⚠ Tallene er målt gennem en agent-proxy / lokalt, ikke fra en dansk forbindelse. Mål igen fra DK før der konkluderes på absolutte tal. | Mads (a: beslut migration) + ejer/Mads (b: plan) |
